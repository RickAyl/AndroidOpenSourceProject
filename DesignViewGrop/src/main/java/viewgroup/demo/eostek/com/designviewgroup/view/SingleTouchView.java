package viewgroup.demo.eostek.com.designviewgroup.view;
...
public class SingleTouchView extends View{
    ...
    private void initConfig(){
        ...
        if(controlDrawable == null){
            controlDrawable = getContext().getResources().getDrawable(R.mipmap.ic_launcher);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() ) {
            case MotionEvent.ACTION_DOWN:
                Log.v("point","touch x : " + event.getX() + " , touch y : " + event.getY());
                Log.v("point","mViewPaddingLeft : " + mViewPaddingLeft + " , mViewPaddingTop : " + mViewPaddingTop);
    ...
    private void computeRect(int left, int top, int right, int bottom, float degree){
        ...
        //图形旋转后，整个view的宽高改变，故取x与y的最大值，求出整个view的宽高
        //计算X坐标最大的值和最小的值
        ...
        //offsetX = minCoordinateX - left;
        offsetY = mViewHeight / 2 - viewCenterPoint.y;
        //offsetY = maxCoordinateY - bottom;
        ...
        /*if(offsetX >= 0){
            ...
            mLTPoint.x = mLTPoint.x + Math.abs(offsetX) - halfDrawableWidth;
            mRTPoint.x = mRTPoint.x - Math.abs(offsetX) + halfDrawableWidth;
            mRBPoint.x = mRBPoint.x - Math.abs(offsetX) + halfDrawableWidth;
            mLBPoint.x = mLBPoint.x + Math.abs(offsetX) - halfDrawableWidth;
        }

        if(offsetY >= 0){
            mLTPoint.y -= (offsetY + halfDrawableHeight);
            mRTPoint.y -= (offsetY + halfDrawableHeight);
            mRBPoint.y = mRBPoint.y + offsetY + halfDrawableHeight;
            mLBPoint.y = mLBPoint.y + offsetY + halfDrawableHeight;
        }else if(offsetY < 0){
            mLTPoint.y = mLTPoint.y + Math.abs(offsetY) - halfDrawableHeight;
            mRTPoint.y = mLTPoint.y + Math.abs(offsetY) - halfDrawableHeight;
            mRBPoint.y = mRBPoint.y - Math.abs(offsetY) + halfDrawableHeight;
            mLBPoint.y = mLBPoint.y - Math.abs(offsetY) + halfDrawableHeight;
        }*/
    ...
    public static Point obtainRoationPoint(Point center, Point source, float degree) {
        ...
        //通过中心点到四个顶点的连线与x正轴的总偏移角度计算旋转后新的点坐标
...
//          image_layout(newPaddingLeft, newPaddingTop, newPaddingLeft + actualWidth, newPaddingTop + actualHeight);
        }
